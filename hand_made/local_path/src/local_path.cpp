...
local_path::local_path()
{
    ...
    pnh.param("getoff",getoff,1.5);
    pnh.param("get",get,1.5);
    ...
    sim_pose = nh.subscribe("/odom1",10,&local_path::sim_pose_callback,this);
    ...
    speed_sub = nh.subscribe("/speed1",10,&local_path::speed_callback,this);
    ...
    if(flag == true)
    {
        ...
        if(temp_num + num1 <= path.poses.size())
        {
            for(int i = temp_num; i< local_path_size; i++)
            {
                if(state.data == "no")
                {
                    if(get_passenger(pose) < get && GO == false)
                    {
                        temp_pose.pose.position.x = global_path.poses.at(i).pose.position.x + 0.3*sin(vehicle_yaw);
                        temp_pose.pose.position.y = global_path.poses.at(i).pose.position.y - 0.3*cos(vehicle_yaw);
                        temp_pose.pose.position.z = 0;
                        tracking_path.poses.push_back(temp_pose);
                    }
                    else if(GO == true && get_off_passenger(pose) < getoff)
                    {
...
int main(int argc, char * argv[])
{
    ...
    local_path path;
    path.flag = false;
    ifstream file_node("/home/wecar/catkin_ws/src/hand_made/Dijkstra/path/node.csv");
